...
int handle_opts(PRJNAME_options_t *opts, int argc, char **argv)
{
    ...
    if (opts->command == cmd_do_main_function)
    {
        ...
        // if (argc - optind != 2) {
        //   fprintf(stderr, "The command requires exactly two arguments:
        //   template "
        //                   "and project name");
        //   return -1;
        // }

        // strncpy(opts->template_name, argv[optind],
        // sizeof(opts->template_name)); strncpy(opts->project_name, argv[optind
        // + 1], sizeof(opts->project_name));
    }
    return 0;
}
